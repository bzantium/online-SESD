...
#Online TRC Sequential ESD Detector
TRCDetector <- function(data, time=1:length(data), dwins, rwins, train_size, alpha, maxr) {
  train_data <- data[1:train_size]
  online_data <- data[(train_size+1):length(data)]
  train_time <- time[1:train_size]
  online_time <- time[(train_size+1):length(data)]
  c_ins <- TCHA$new(data=train_data, time=train_time, wins=dwins)
  r_ins <- TRES$new(data=train_data, time=train_time, wins=rwins)
  SESD_TCHA <- SESD$new(data=c_ins$tcha, alpha=alpha, maxr=maxr)
  SESD_TRES <- SESD$new(data=r_ins$tres, alpha=alpha, maxr=maxr)
  anomal_index <- c()
